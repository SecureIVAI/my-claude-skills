...
- **Global scope**: `/workspaces/my-claude-skills/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/workspaces/my-claude-skills/.claude/notes/`)
...
2. Glob `/workspaces/my-claude-skills/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/workspaces/my-claude-skills/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/workspaces/my-claude-skills/.claude/notes/` — works in any directory
