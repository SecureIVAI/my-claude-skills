...
```bash
AUDITOR_MODEL=$(node "/workspaces/my-claude-skills/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-nyquist-auditor --raw)
NYQUIST_CFG=$(node "/workspaces/my-claude-skills/.claude/get-shit-done/bin/gsd-tools.cjs" config get workflow.nyquist_validation --raw)
```
...
```
Task(
  prompt="Read /workspaces/my-claude-skills/.claude/agents/gsd-nyquist-auditor.md for instructions.\n\n" +
...
**State B (create):**
1. Read template from `/workspaces/my-claude-skills/.claude/get-shit-done/templates/VALIDATION.md`
...
node "/workspaces/my-claude-skills/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-${PHASE}): add/update validation strategy"
```
